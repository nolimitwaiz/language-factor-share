...
if [ "$SLURM_ARRAY_TASK_ID" -eq 0 ]; then
  python code/cluster_grid.py --model "$M" --n_sents 300 --batch_size 16 --dtype fp16 --device cuda
else
  lm_eval --model hf --model_args "pretrained=$M,dtype=float16" --tasks belebele \
    --num_fewshot 0 --limit 300 --batch_size auto:4 \
    --output_path "results/belebele0/SmolLM2-1.7B" --trust_remote_code
fi
