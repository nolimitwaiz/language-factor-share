...
#SBATCH --array=0-7
#SBATCH --output=cluster/logs/e312313_%A_%a.log
...
if [ "$SLURM_ARRAY_TASK_ID" -lt 4 ]; then
    python code/lens_312.py --model "${MODELS[$IDX]}" --device cuda
else
    python code/transfer_313.py --model "${MODELS[$IDX]}" --device cuda
fi
